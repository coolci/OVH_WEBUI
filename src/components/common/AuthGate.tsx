...
function LoginOverlay({
  ...
  const [key, setKey] = useState(() => getApiSecretKey() || "");
  ...
  return (
    <div className="fixed inset-0 z-[100] app-canvas flex items-center justify-center px-4">
      <div className="w-full max-w-md terminal-card p-7 sm:p-8 space-y-5 shadow-soft">
        <div className="flex items-center gap-3">
          <div className="icon-well w-11 h-11">
            <ShieldAlert className="w-5 h-5 text-primary" />
          </div>
          <div>
            <h2 className="text-lg font-semibold leading-tight tracking-tight">需要 API 密钥</h2>
            <p className="text-[12px] text-muted-foreground mt-0.5">访问控制台前请先验证身份</p>
          </div>
        </div>
...
/**
 * 通过裸 axios 探测 /api/stats（受保护端点，不走带 toast 的拦截器）：
 * - 200 → key 有效
 * - 401 → key 无效（返回 false）
 * - 其它（网络 / 5xx） → 抛错让调用方决定
 * 支持 localStorage backendUrl
 */
