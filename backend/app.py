...
# Queue processor runtime state
queue_processor_running = True
queue_processor_processed_count = 0
queue_processor_last_process_time = None
...
def update_stats():
    ...
    stats = {
        ...
        "queueProcessorRunning": queue_processor_running,
        "monitorRunning": monitor_running  # 监控器实际运行状态
    }

def _merge_datacenters(datacenters):
    priority = {
        "available": 0,
        "1H": 0,
        "24H": 1,
    ...
    for dc in datacenters:
        ...
        if not existing:
            merged[name] = dc
            continue
        existing_availability = existing.get("availability", "unknown")
        if priority.get(availability, 4) < priority.get(existing_availability, 4):
            merged[name] = dc
...
# Process queue items
def process_queue():
    global deleted_task_ids, queue_processor_running, queue_processor_processed_count, queue_processor_last_process_time
    # 并发处理配置
    ...
    while True:
        ...
        if items_ready_to_process:
            ...
            if processed_items:
                ...
                queue_processor_processed_count += len(processed_items)
                queue_processor_last_process_time = int(time.time())
...
def load_server_list():
    ...
    try:
        ...
        for plan in catalog.get("plans", []):
            ...
            datacenters = _merge_datacenters(datacenters)
...
def get_queue_processor_status():
    return jsonify({
        ...
        "processedCount": queue_processor_processed_count,
        "lastProcessTime": queue_processor_last_process_time
    })

@app.route('/api/queue/processor/start', methods=['POST'])
def start_queue_processor_api():
    global queue_processor_running
    queue_processor_running = True
    update_stats()
    return jsonify({"status": "success", "message": "Queue processor started"})

@app.route('/api/queue/processor/stop', methods=['POST'])
def stop_queue_processor_api():
    global queue_processor_running
    queue_processor_running = False
    update_stats()
    return jsonify({"status": "success", "message": "Queue processor stopped"})
...
@app.route('/api/vps-monitor/subscriptions/<subscription_id>', methods=['PUT'])
def update_vps_subscription(subscription_id):
    """更新VPS订阅"""
    ...
    if "datacenters" in data:
        subscription["datacenters"] = data.get("datacenters") or []
    if "monitorLinux" in data:
        subscription["monitorLinux"] = bool(data.get("monitorLinux"))
    if "monitorWindows" in data:
        subscription["monitorWindows"] = bool(data.get("monitorWindows"))
    if "notifyAvailable" in data:
        subscription["notifyAvailable"] = bool(data.get("notifyAvailable"))
    if "notifyUnavailable" in data:
        subscription["notifyUnavailable"] = bool(data.get("notifyUnavailable"))
    if "autoOrder" in data:
        subscription["autoOrder"] = bool(data.get("autoOrder"))

    save_vps_subscriptions()
    return jsonify({"status": "success", "subscription": subscription})
...
@app.route('/api/ovh/account/balance', methods=['GET'])
def get_account_balance():
    """获取OVH账户余额 - GET /me/balance"""
    client = get_ovh_client()
    if not client:
        return jsonify({"status": "error", "message": "未配置OVH API"}), 400

    try:
        balance = client.get('/me/balance')
        add_log("INFO", "成功获取账户余额", "account_management")
        return jsonify({
            "status": "success",
            "data": balance
        })
    except Exception as e:
        add_log("ERROR", f"获取账户余额失败: {str(e)}", "account_management")
        return jsonify({
            "status": "error",
            "message": f"获取账户余额失败: {str(e)}"
        }), 500

@app.route('/api/ovh/account/orders', methods=['GET'])
def get_account_orders():
    ...
    try:
        ...
        order_ids = client.get('/me/order')
        orders = []
        for order_id in order_ids[:limit]:
            try:
                order_detail = client.get(f'/me/order/{order_id}')
                orders.append(order_detail)
            except Exception as e:
                add_log("WARNING", f"获取订单 {order_id} 详情失败: {str(e)}", "account_management")

        add_log("INFO", f"成功获取 {len(orders)} 条订单记录", "account_management")
        return jsonify({
            "status": "success",
            "data": orders
        })
    except Exception as e:
        add_log("ERROR", f"获取订单列表失败: {str(e)}", "account_management")
        return jsonify({
            "status": "error",
